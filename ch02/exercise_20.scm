...
(define (same-parity head . tail)
  ...
  (define (search pattern items)
    (cond ((null? items) items)
          ((pattern (car items))
           (append (list (car items)) (search pattern (cdr items))))
          (else (search pattern (cdr items)))))

  (if (even? head)
      (append (list head) (search even? tail))
      (append (list head) (search odd? tail))))
