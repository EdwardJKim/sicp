...
; the Miller-Rabin test (Miller 1976; Rabin 1980). This starts from an
...
; equal to 1 or n − 1 whose square is equal to 1 modulo n. It is possible to
; prove that if such a nontrivial square root of 1 exists, then n is not prime.
; It is also possible to prove that if n is an odd number that is not prime,
; then, for at least half the numbers a < n, computing a^(n − 1) in this way
...
(define (miller-rabin-prime? n)
  ...
  (define (miller-rabin-iter a)
    (cond ((= a 1) true)
          ((fermat-test a) (miller-rabin-iter (- a 1)))
          (else false)))

  (cond ((< n 3) true)
        (else (miller-rabin-iter (- n 2)))))
