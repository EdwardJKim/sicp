...
; state variables a and b in the fib-iter process of 1.2.2: a <- a + b and
...
; T_pq, where T_pq transforms the pair (a, b) according to a <- bq + aq + ap
; and b <- bp + aq. Show that if we apply such a transformation T_pq twice, the
...
;    = b pq + a q^2 + b q^2 + a q^2 + a pq + b pq + a pq + a p^2
